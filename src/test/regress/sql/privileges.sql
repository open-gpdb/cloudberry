...
-- autovacuum workers come and go on their own, so leave them out to keep the
-- set of reported background processes stable
SELECT backend_type FROM pg_stat_activity
WHERE backend_type IS DISTINCT FROM 'autovacuum worker'
