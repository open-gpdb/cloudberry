...
# Ordinary DML revokes the all-visible and all-frozen status of the pages
# it touches; this is delivered with the regular relation statistics.
# vacuum_freeze_min_age is 0 here, so the pages the vacuum above marked
# all-visible were marked all-frozen as well.
...
# delay_time is the share of the run that went into the cost-based vacuum
# delay.  Nothing sleeps with the default vacuum_cost_delay of 0.
is( $node->safe_psql(
		'postgres',
		'SELECT delay_time = 0 '
		  . "FROM pg_stat_vacuum_tables WHERE relname = 'vestat'"),
	't',
	'nothing is delayed without a cost delay');

$node->safe_psql('postgres',
	'INSERT INTO vestat SELECT g FROM generate_series(1, 10000) g');
$node->safe_psql('postgres', 'DELETE FROM vestat');
$node->safe_psql('postgres', 'ALTER SYSTEM SET vacuum_cost_delay = 1');
$node->safe_psql('postgres', 'ALTER SYSTEM SET vacuum_cost_limit = 1');
$node->reload;
$node->poll_query_until('postgres', 'SHOW vacuum_cost_limit', '1')
  or die 'vacuum_cost_limit was not changed';
$node->safe_psql('postgres', 'VACUUM vestat');
$node->safe_psql('postgres', 'ALTER SYSTEM RESET vacuum_cost_delay');
$node->safe_psql('postgres', 'ALTER SYSTEM RESET vacuum_cost_limit');
$node->reload;
ok( $node->poll_query_until(
		'postgres',
