...
--
-- Cluster-wide views: the same information from the coordinator and from
...
$$
    SELECT pg_catalog.gp_execution_segment() AS gp_segment_id,
        ...
        @extschema@.pg_stat_get_vacuum_delay_time(c.oid) / 1000.0
    FROM pg_catalog.pg_class c
        LEFT JOIN pg_catalog.pg_namespace n ON n.oid = c.relnamespace
    WHERE c.relkind IN ('r', 't', 'm')
$$
...
$$
    SELECT pg_catalog.gp_execution_segment() AS gp_segment_id,
        ...
        @extschema@.pg_stat_get_vacuum_delay_time(i.oid) / 1000.0
    FROM pg_catalog.pg_class c
        JOIN pg_catalog.pg_index x ON c.oid = x.indrelid
        JOIN pg_catalog.pg_class i ON i.oid = x.indexrelid
        LEFT JOIN pg_catalog.pg_namespace n ON n.oid = c.relnamespace
...
$$
    SELECT pg_catalog.gp_execution_segment() AS gp_segment_id,
        ...
        @extschema@.pg_stat_get_db_vacuum_delay_time(d.oid) / 1000.0
    FROM pg_catalog.pg_database d
$$
LANGUAGE SQL EXECUTE ON COORDINATOR;
